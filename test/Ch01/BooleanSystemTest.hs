...
tests =
  testGroup
    ...
    [ testCase "Joinable" $ do
        let joinTest x y expected = join (BooleanSystem x) (BooleanSystem y) @=? BooleanSystem expected
        joinTest False False False
        joinTest False True True
        joinTest True False True
        joinTest True True True,
      testCase "Ord" $ do
        let ordTest x y expected = (BooleanSystem x) <= (BooleanSystem y) @=? expected
